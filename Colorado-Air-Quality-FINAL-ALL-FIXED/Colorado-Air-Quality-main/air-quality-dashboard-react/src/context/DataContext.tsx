...
export const DataProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
  ...
  const fetchAllAirQualityData = async () => {
    ...
    try {
      ...
      // Generate fallback data for all cities first to ensure we have data
      for (const city of COLORADO_CITIES) {
        features.push({
          ...
          properties: {
            city: city.name,
            zip: city.zip,
            AQI: Math.floor(Math.random() * 50) + 20, // Fallback random AQI between 20-70
            Pollutant: 'PM2.5',
            category: 'Good',
      ...
      // Set initial data with fallbacks to ensure we have something to display
      setAirQualityData({ 
        type: 'FeatureCollection', 
        features: features 
      });
      
      // Calculate initial rankings with fallback data
      const initialRankings = calculateCityRankings({ type: 'FeatureCollection', features: features });
      if (initialRankings) {
        setMostPollutedCities(initialRankings.mostPolluted);
        setCleanestCities(initialRankings.cleanest);
      }
      
      // Now try to fetch real data
      for (let i = 0; i < COLORADO_CITIES.length; i++) {
        const city = COLORADO_CITIES[i];
        try {
          ...
          // Use PM2.5 if available, otherwise use ozone, otherwise keep fallback
          const primaryData = pm25Data || ozoneData;
          
          if (primaryData) {
            // Update the feature with real data
            features[i] = {
              type: 'Feature',
              geometry: {
                type: 'Point',
          ...
          // Keep the fallback data for this city
        }
      ...
      // Update with final data (mix of real and fallback)
      setAirQualityData({ 
        type: 'FeatureCollection', 
        features: features 
      });
      
      // Recalculate rankings with final data
      const finalRankings = calculateCityRankings({ type: 'FeatureCollection', features: features });
      if (finalRankings) {
        setMostPollutedCities(finalRankings.mostPolluted);
        setCleanestCities(finalRankings.cleanest);
      }
  ...
  const fetchAsthmaData = async () => {
    try {
      // Try to get real asthma data from CDC API
      ...
      // Try to get emergency visit data
      try {
        const currentYear = new Date().getFullYear();
        const emergencyData = await fetchAsthmaEmergencyVisits(currentYear - 1, currentYear);
        const hospitalizationData = await fetchAsthmaHospitalizations(currentYear - 1, currentYear);
        
        if (emergencyData.length > 0) {
          setAsthmaEventData(emergencyData);
          setAsthmaStatistics(prev => ({
            ...prev,
            eventDataAvailable: true,
            emergencyVisitRate: emergencyData.reduce((sum, d) => sum + d.rate, 0) / emergencyData.length
          }));
        } else {
          // Use mock emergency data
          const mockEmergencyData = getMockAsthmaEmergencyData([]);
          setAsthmaEventData(mockEmergencyData);
          setAsthmaStatistics(prev => ({
            ...prev,
            eventDataAvailable: true,
            emergencyVisitRate: mockEmergencyData.reduce((sum, d) => sum + d.rate, 0) / mockEmergencyData.length
          }));
        }
      } catch (eventError) {
        console.error('Error fetching asthma event data:', eventError);
        // Use mock emergency data as fallback
        const mockEmergencyData = getMockAsthmaEmergencyData([]);
        setAsthmaEventData(mockEmergencyData);
      }
      ...
      if (data.length > 0 && data[0].emergencyVisitRate !== null) {
        ...
        if (pollutionData.length > 0 && asthmaEmergencyData.length > 0) {
          ...
          timeLagAnalysis.forEach(item => {
            if (Math.abs(item.correlation) > Math.abs(maxCorr)) {
  ...
  // Recalculate rankings when airQualityData changes
  useEffect(() => {
    if (airQualityData.features && airQualityData.features.length > 0) {
      const rankings = calculateCityRankings(airQualityData);
      if (rankings) {

...
class RealDataService {
  ...
  async fetchAirQualityData(zip: string): Promise<AirQualityData> {
    try {
      // Use the correct AirNow API endpoint format
      const apiUrl = `${AIRNOW_BASE_URL}/observation/zipCode/current/?format=application/json&zipCode=${zip}&distance=50&API_KEY=${AIRNOW_API_KEY}`;
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        // Find the most recent observation with the highest AQI (as per AirNow documentation)
        const validObservations = data.filter(obs => obs.AQI && obs.AQI > 0);
        if (validObservations.length > 0) {
          const observation = validObservations.reduce((prev, current) => 

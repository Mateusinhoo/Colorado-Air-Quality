...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadData = async () => {
    ...
    try {
      // Load air quality data for all zip codes
      const airQualityPromises = zipCodes.slice(0, 15).map(async (zip) => {
      ...
      // Load city rankings
      const [pollutedCities, cleanCities] = await Promise.all([
        realDataService.getMostPollutedCities(),
        realDataService.getCleanestCities()
      ]);

      setMostPollutedCities(pollutedCities.map(city => ({
        name: city.city,
        zip: city.zip,
        aqi: city.aqi,
      ...
      setCleanestCities(cleanCities.map(city => ({
        name: city.city,
        zip: city.zip,
        aqi: city.aqi,
        pollutant: city.pollutant,
      ...
      const asthmaStats = await realDataService.getAsthmaStatistics();
      setAsthmaStatistics({
        averagePrevalence: asthmaStats.averageAsthmaRate,
        totalCounties: 10,
        dataAvailable: true
      });
